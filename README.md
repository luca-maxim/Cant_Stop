...
- [`Study Data/`](Study%20Data/) — the anonymized study dataset ([`data.csv`](Study%20Data/data/data.csv)) and the R analysis script ([`Analysis.R`](Study%20Data/Analysis.R)) used to produce the paper's results.
...
The statistical analysis (mixed models, Bayesian modeling with `brms`) is in [`Study Data/Analysis.R`](Study%20Data/Analysis.R) and reads [`Study Data/data/data.csv`](Study%20Data/data/data.csv). Open it in RStudio and install the required packages:
```r
install.packages(c("tidyverse", "lme4", "lmerTest", "mixedpower", "performance",
                    "report", "readr", "rstudioapi", "emmeans", "brms",
                    "bayestestR", "xtable", "tidybayes"))
```
